...
export class Pm10Component implements OnInit, AfterViewInit, OnDestroy {
  data: any[] = [];
  chartData: any[] = [];
  chart: any;
  avg: number = 0;
  ...
  private appendToChart(newRecords: any[]) {
    ...
    todayRecords.forEach(r => {
      ...
      this.chart.data.datasets[0].data.push(r.pm10);
    });
  ...
  private computeStats(data: any[]) {
    ...
    const vals = data.map(r => Number(r.pm10) || 0);
