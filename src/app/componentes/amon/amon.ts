...
@Component({
  ...
  templateUrl: './amon.html',
  styleUrls: ['./amon.css']
})
export class AmonComponent implements OnInit, AfterViewInit, OnDestroy {
  data: any[] = [];
  chartData: any[] = [];
  chart: any;
  avg: number = 0;
  ...
  private initChart(data: any[]) {
    const ctx = document.getElementById('amonChart') as HTMLCanvasElement;
    if (!ctx) return;
