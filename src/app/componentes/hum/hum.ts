...
@Component({
  ...
  templateUrl: './hum.html',
  styleUrls: ['./hum.css']
})
export class HumComponent implements OnInit, AfterViewInit, OnDestroy {
  data: any[] = [];
  chartData: any[] = [];
  chart: any;
  avg: number = 0;
  ...
  private initChart(data: any[]) {
    const ctx = document.getElementById('humChart') as HTMLCanvasElement;
    if (!ctx) return;
